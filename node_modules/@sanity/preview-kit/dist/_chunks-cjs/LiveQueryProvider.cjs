"use strict";
var jsxRuntime = require("react/jsx-runtime"), csm = require("@sanity/client/csm"), mendoza = require("mendoza"), require$$0 = require("react"), hooks = require("./hooks.cjs"), comlink = require("@sanity/comlink"), presentationComlink = require("@sanity/presentation-comlink"), s = { 0: 8203, 1: 8204, 2: 8205, 3: 8290, 4: 8291, 5: 8288, 6: 65279, 7: 8289, 8: 119155, 9: 119156, a: 119157, b: 119158, c: 119159, d: 119160, e: 119161, f: 119162 }, c = { 0: 8203, 1: 8204, 2: 8205, 3: 65279 };
...
function LiveStoreProvider(props) {
  ...
  const perspective = usePerspective(props.perspective || "previewDrafts"), [client] = require$$0.useState(() => {
  ...
  const [subscriptions, setSubscriptions] = require$$0.useState([]), [snapshots] = require$$0.useState(() => /* @__PURE__ */ new Map()), hooks$1 = useHooks(setSubscriptions), [context] = require$$0.useState(() => function(initialSnapshot, query, params) {
    const key = getQueryCacheKey(query, params);
    return snapshots.has(key) || snapshots.set(key, {
      result: initialSnapshot,
      resultSourceMap: {}
    }), { subscribe: (onStoreChange) => {
      const unsubscribe = hooks$1.subscribe(key, query, params, onStoreChange);
      return () => unsubscribe();
    }, getSnapshot: () => snapshots.get(key)?.result };
  }), [turboIds, setTurboIds] = require$$0.useState([]), [docsInUse] = require$$0.useState(() => /* @__PURE__ */ new Map()), turboIdsFromSourceMap = require$$0.useCallback(
    (contentSourceMap) => {
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(hooks.defineStoreContext.Provider, { value: context, children: [
    ...
    subscriptions.map((key) => {
      if (!hooks$1.cache.has(key)) return null;
      const { query, params, listeners } = hooks$1.cache.get(key);
...
function useHooks(setSubscriptions) {
  const [cache] = require$$0.useState(() => /* @__PURE__ */ new Map()), subscribe = require$$0.useCallback(
    (key, query, params, listener) => {
      cache.has(key) || (cache.set(key, { query, params, listeners: /* @__PURE__ */ new Set() }), require$$0.startTransition(
      ...
      return listeners.add(listener), () => {
        listeners.delete(listener), listeners.size === 0 && (cache.delete(key), require$$0.startTransition(
  ...
  return require$$0.useMemo(() => ({ cache, subscribe }), [cache, subscribe]);
}
function Turbo(props) {
  const { client, snapshots, cache, turboIds, setTurboIds, docsInUse, perspective } = props, { projectId, dataset } = require$$0.useMemo(() => {
    const { projectId: projectId2, dataset: dataset2 } = client.config();
    return { projectId: projectId2, dataset: dataset2 };
  }, [client]);
  require$$0.useEffect(() => {
    ...
    JSON.stringify(turboIds) !== JSON.stringify(nextTurboIdsSnapshot) && require$$0.startTransition(() => setTurboIds(nextTurboIdsSnapshot));
  }, [cache, docsInUse, setTurboIds, snapshots, turboIds]);
  const [batch, setBatch] = require$$0.useState([]);
  require$$0.useEffect(() => {
    ...
    nextBatchSlice.length !== 0 && require$$0.startTransition(() => setBatch((prevBatch) => [...prevBatch.slice(-100), nextBatchSlice]));
  }, [batch, dataset, projectId, turboIds]);
  const [lastMutatedDocumentId, setLastMutatedDocumentId] = require$$0.useState();
  return require$$0.useEffect(() => {
    const subscription = client.listen(
      ...
      require$$0.startTransition(() => setLastMutatedDocumentId(update.documentId));
    });
    return () => subscription.unsubscribe();
  }, [client, dataset, projectId]), require$$0.useEffect(() => {
    ...
    require$$0.startTransition(() => setLastMutatedDocumentId(void 0));
  }, [cache, dataset, lastMutatedDocumentId, perspective, projectId, snapshots, turboIds]), /* @__PURE__ */ jsxRuntime.jsx(jsxRuntime.Fragment, { children: batch.map((ids) => /* @__PURE__ */ jsxRuntime.jsx(
    GetDocuments,
    {
      client,
      projectId,
...
function turboChargeResultIfSourceMap(projectId, dataset, result, perspective, resultSourceMap) {
  return resultSourceMap ? csm.applySourceDocuments(
    result,
    resultSourceMap,
    (sourceDocument) => {
